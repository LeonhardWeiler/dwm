  ...
  for (i = 0; i < LENGTH(tags); i++) {
    if (showtags) {
      if (m->tagset[m->seltags] & 1 << i) {
        // Selected tag: Use SchemeTagsSel
        w = TEXTW("");  // Change "" to your preferred icon or text
        drw_setscheme(drw, scheme[SchemeTagsSel]);
        drw_text(drw, x, 0, w, bh, lrpad / 2, "", urg & 1 << i);  // Use the icon or text here
      } else {
        // Unselected tag: Use SchemeTagsNorm
        w = TEXTW(tags[i]);
        drw_setscheme(drw, scheme[SchemeTagsNorm]);
        drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
      }

      // Optionally draw a rectangle for occupied and floating windows
